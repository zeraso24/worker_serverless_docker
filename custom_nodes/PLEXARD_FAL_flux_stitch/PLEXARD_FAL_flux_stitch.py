"""
Flux Kontext LoRA Inpaint Node
A dedicated node for Flux Kontext LoRA inpainting functionality.
...
# ==================== FLUX KONTEXT INPAINT NODE ====================


class FluxKontextInpaint:
    """
    Flux Kontext Inpaint Node
    Performs inpainting using FLUX.1 Kontext model.
    Requires: image, mask, and prompt.
    Optional: reference_image for guided inpainting.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "reference_image": ("IMAGE", {"tooltip": "Reference image for inpainting (optional)"}),
                "api_key": ("STRING", {"default": "", "tooltip": "FAL API key (optional, overrides config.ini and environment variable)"}),
                "num_inference_steps": ("INT", {"default": 40, "min": 1, "max": 100, "step": 1, "tooltip": "Number of inference steps"}),
                "guidance_scale": ("FLOAT", {"default": 2.5, "min": 0.0, "max": 20.0, "step": 0.1, "tooltip": "CFG scale"}),
                "strength": ("FLOAT", {"default": 0.88, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "Strength of inpainting"}),
                "num_images": ("INT", {"default": 1, "min": 1, "max": 4, "step": 1, "tooltip": "Number of images to generate"}),
                "seed": ("INT", {"default": -1, "min": -1, "max": 2147483647, "tooltip": "Seed for reproducibility (-1 for random)"}),
                "width": ("INT", {"default": 1024, "min": 256, "max": 4096, "step": 8, "tooltip": "Output image width"}),
                "height": ("INT", {"default": 1024, "min": 256, "max": 4096, "step": 8, "tooltip": "Output image height"}),
            },
        }
    ...
    DESCRIPTION = "Flux Kontext Inpaint - Inpainting with reference images"
    ...
    def inpaint(
        ...
        """
        Perform inpainting using Flux Kontext model.
        
        Args:
            image: Input image tensor
            mask: Mask tensor (will be converted to image if needed)
            prompt: Text prompt
            reference_image: Reference image tensor (optional)
            ...
            strength: Inpainting strength (default: 0.88)
        ...
        """
        model_name = "Flux Kontext Inpaint"
        ...
        try:
            ...
            # Validate num_images (critical - must be between 1 and 4)
            if num_images is None or not isinstance(num_images, (int, float)):
                num_images = 1
            num_images = int(max(1, min(4, num_images)))
            
            # Validate seed (handle None and convert to -1 if invalid)
            if seed is None:
                seed = -1
                ...
                seed = int(seed)
                # If seed is out of valid range, set to -1 (random)
            ...
            if width is None or not isinstance(width, (int, float)):
                width = 1024
            width = int(max(256, min(4096, width)))
            # Round to nearest multiple of 8
            width = (width // 8) * 8
            
            if height is None or not isinstance(height, (int, float)):
                height = 1024
            height = int(max(256, min(4096, height)))
            # Round to nearest multiple of 8
            height = (height // 8) * 8
        except Exception as e:
        ...
        if reference_image is not None:
            reference_image_url = ImageUtils.upload_image(reference_image, api_key=api_key_for_upload)
            if not reference_image_url:
                print(f"Warning: Failed to upload reference image for {model_name}, continuing without it")
                reference_image_url = None

        # Build API arguments according to fal.ai API schema
        arguments = {
            "image_url": image_url,
            "mask_url": mask_url,
            "prompt": prompt,
        ...
        # Add reference_image_url only if provided
        if reference_image_url is not None:
            arguments["reference_image_url"] = reference_image_url

        # Add seed only if it's valid (not -1 or None)
        if seed is not None and seed != -1 and isinstance(seed, int):
            arguments["seed"] = seed
        
        # Add image size (user-defined width and height)
        # Note: Some fal.ai endpoints may accept image_size parameter
        ...
        try:
            result = ApiHandler.submit_and_get_result(
                "fal-ai/flux-kontext-lora/inpaint", arguments, api_key=api_key_for_upload
            )
...
NODE_CLASS_MAPPINGS = {
    "FluxKontextInpaint": FluxKontextInpaint,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "FluxKontextInpaint": "Flux Kontext LoRA Inpaint",
}
